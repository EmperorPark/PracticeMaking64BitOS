...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
; MIN64 OS에 관련된 환경 설정 값
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
TOTALSECORCOUNT: dw 1    ; 부트 로더를 제외한 MINT64 OS 이미지 크기 최대 1152 섹터(0x9000Bytes)까지 가능
...
START:
    mov ax, 0x07C0  ; 부트로더의 시작 Address(0x7C00)를 세그먼트 레지스터 값으로 변환
    mov ds, ax      ; DS 세그먼트 레지스터에 설정
    mov ax, 0xB800  ; 비디오 메모리의 시작 Address (0xB800)를 세그먼트 레지스터 값으로 변환
...
.SCREENCLEARLOOP:                   ; 화면을 지우는 루프
    mov byte [ es: si ], 0          ; 비디오 메모리의 문자가 위치하는 어드레서에 0을 복사하여 문자를 삭제
    mov byte [ es: si + 1 ], 0x0A   ; 비디오 메모리의 속성이 위치하는 어드레스에 0x0A(검은 바탕에 밝은 녹색)을 복사

    add si, 2                       ; 문자와 속성을 설정했으므로 다음 위치로 이동

    cmp si, 80 * 25 * 2             ; 화면 전체 크기는 80문자 * 25라인(각 문자마다 바탕 속성, 글자 속성 2byte)
                                    ; 출력한 문자 수를 의미하는 SI레지스터와 비교
    jl .SCREENCLEARLOOP             ; SI레지스터가 80 * 25 * 2보다 작다면 아직 CLEAR못한 영역이 있으므로 .SCREENCLEARLOOP레이블로 이동
...
RESETDISK:                  ; 디스크를 리셋하는 코드의 시작
    ...
    mov bx, 0x0000              ; BX 레지스터에 0x0000을 설정하여 복사할 어드레스를 0x1000:0000(0x10000)으로 최종설정

    mov di, word[ TOTALSECORCOUNT ] ; 복사할 OS 이미지의 섹터 수를 DI 레지스터에 설정
...
READDATA:                   ; 디스크를 읽는 코드의 시작
    ...
    jc HANDLEDISKERROR              ; ERROR 발생 시 HANDLEDISKERROR로 이동

    
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ; 복사할 어드레스와 트랙, 헤드, 섹터 어드레스 계산 
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    add si, 0x0020  ; 512(0x200)Bytes만큼 읽었으므로, 이를 세그먼트 레지스터 값으로 변환
    mov es, si      ; ES 세그먼트 레지스터에 더해서 어드레스를 한섹터 만큼 증가
...
PRINTMESSAGE:
    ...
    mov bp, sp  ; 베이스 포인터 레지스터(BP)에 스택 포인터레지스터(SP)의 값을 설정
                ; 베이스 포인터 레지스터(BP)를 이용해서 파라미터에 접근할 목적

    push es     ; ES 세그먼트 레지스터 부터 DX레지스터까지 스택에 삽입 함수에서 임시로 사용하는 레지스터로 
    push si     ; 함수의 마지막 부분에서 스택에 삽입된 값을 꺼내 원래 값으로 복원
    push di
    push ax
    push cx
    push dx

    ; ES 세그먼트 레지스터에 비디오모드 어드레스 설정
    mov ax, 0xB800              ; 비디오 메모리 시작 어드레스(0x0B8000)를 세그먼트 레지스터 값으로 변환 ES 세그먼트 레지스터에 설정
    mov es, ax                  ; ES 세그먼트 레지스터에 서ㅗㄹ정

    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ;; X, Y의 좌표로 비디오 메모리 어드레스를 계산 함
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
    ; Y 좌표를 이용해 먼저 라인 어드레스를 구함
    mov ax, word [bp + 6]   ; 파라미터 2(화면 좌표 Y)를 AX 레지스터에 설정
    mov si, 160             ; 한 라인의 바이트 수(2 * 80 컬럼)를 SI 레지스터에 설정
    mul si                  ; AX 레지스터와 SI레지스터를 곱하여 화면 Y 어드레스 계산
    ...
    add di, ax              ; 화면 Y의 어드레스와 계산된 X 어드레스를 더해서 실제 비디오 메모리 어드레스를 계산

    ; 출력할 문자열의 어드레스
    mov si, word [bp + 8]   ; 파라미터 3(출력할 문자열의 어드레스)

.MESSAGLOOP:                        ; 메세지를 출력하는 루프
    mov cl, byte [ si ]  ; MESSAGE1의 Address에서 SI 레지스터 값만큼 더한 위치의 문자를 CL레지스터에 복사
                                    ; CL레지스터는 CX레지스터의 하위 1Byte를 의미(문자하나는 1Byte크기만 씀)
    ...
    add si, 1                       ; SI 레지스터에 1더해 다음 문자열로 이동
    add di, 2                       ; DI 레지스터에 2를 더해 비디오 메모리의 다음문자 위치로 이동(비디오 메모리는 바탕, 속성 쌍으로 구성되므로 문자만 출력하려면 2를 더해야 함)

    jmp .MESSAGLOOP                 ; 메세지 출력 루프로 이동하여 다음 문자를 출력

.MESSAGEEND:
    pop dx      ; 함수에서 사용이 끝난 DX 레지스터 부터 ES레지스터까지를 스택에 삽입된 값을 이용해서 복원
    pop cx      ; 스택은 가장 마지막에 들어간 데이터가 가장 먼저 나오는 자료구조이므로 삽입의 역순으로 제거(pop)해야 함
    pop ax
    pop di
    pop si
    pop es
...
;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
;부트 로더 시작 메시지
MESSAGE1:               db 'MINT64 OS Boot Loader Start~!!', 0  ; 출력할 메세지 정의 ; 마지막은 0으로 설정해 .MESSAGLOOP에서 문자열이 종료되었음을 알 수 있도록 함

DISKERRORMESSAGE:       db 'DISK Error~!!', 0
IMAGELOADINGMESSAGE:    db 'OS Image Loding...', 0
LOADINGCOMPLETEMESSAGE: db 'Complete', 0

; 디스크 읽기에 관련된 변수
SECTORNUMBER:           db 0x02     ; OS 이미지가 시작하는 섹터 번호를 저장하는 영역
HEADNUMBER:             db 0x00     ; OS 이미지가 시작하는 헤드 번호를 저장하는 영역
TRACKNUMBER:            db 0x00     ; OS 이미지가 시작하는 트랙 번호흫 저장하는 영역

times 510 - ( $ - $$ )   db 0x00     ; $ 현재 라인의 Address
                                    ; $$: 현재 section(.text)의 시작 Address
                                    ; $ - $$: 현재 섹션을 기준으로 하는 오프셋
                                    ; 510 - ( $ - $$ ): 현재부터 Address 510까짖
                                    ; db 0x00: 1Byte를 선언 값은 0x00
                                    ; time: 반복 수행
                                    ; 현재 위치에서 어드레스 510까지 0x00으로 채움

db 0x55         ; 1Byte를 선언하고 값은 0x55
db 0xAA         ; 1Byte를 선언하고 값은 0xAA
                ; Address 511, 512에 0x55, 0xAA를 써서 부트 섹터로 표기
