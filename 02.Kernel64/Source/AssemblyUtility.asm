...
kInPortByte:
    push rdx        ; 함수에서 임시로 사용하는 레지스터를 스택에 저장
                    ; 함수에서 마지막 부분에서 스택에 삽입된 값을 꺼내 복원
    
    mov rdx, rdi    ; RDX 레지스터에 파라미터 1(포트 번호)를 저장
    mov rax, 0      ; RAX 레지스터를 초기화
    in al, dx       ; DX 레지스터에 저장된 포트 어드레스에서 한 바이트를 출력 ; in: 포트 I/O 어드레스에서 데이터를 읽어오는 명령어
                    ; AL 레지스터에 저장, AL 레지스터를는 함수의 반환 값으로 사용됨
...
kOutPortByte:
    ...
    out dx, al      ; DX 레지스터에 저장된 포트 어드레스에 AL 레지스터에 저장된 한 바이트를 씀; out: 포트 I/O 어드레스에 데이터를 쓰는 명령어
...
kReadTSC:
    ...
    or rax, rdx     ; 하위 32비트 TSC 값을 OR하여 RAX 레지스터에 64비트 TSC 값을 저장하여 반환
...
; Current Context에 현재 콘텍스트를 저장하고 Next Task에서 콘텍스트 복구
...
kSwitchContext:
    ...
    pushfq          ; 아래의 cmp의 결과로 RFLAGS 레지스터가 변하지 않도록 스택에 저장 ; RFLAGS 레지스터는 직접 접근할 수 없으므로 스택에 저장해서 사용
    cmp rdi, 0      ; Current Context가 NULL이면 콘텍스트 복원으로 바로 이동
    je .LoadContext
    popfq           ; ; RFLAGS 레지스터는 직접 접근할 수 없으므로 스택에 저장해서 사용
    ...
    mov rax, rbp                            ; RBP에 저장된 RSP 레지스터를 저장
    ...
    mov qword[ rdi + ( 19 * 8 ) ], rax      ; 다음 콘텍스트 복원 시에 이 함수를 호출한 위치로 이동하게 함
                                            ; rbp + 8 => Return Address가 저장된 위치

    ; 저장한 레지스터를 복구한 후 인터럽트가 발생했을 때 처럼 나머지 콘텍스트를 모두 저장
    pop rax
    pop rbp

    ; 가장 끝부분에 SS, RSP, RFLAGS, CS, RIP 레지스터를 저장했으므로, 이전 영역에 push 명령어로 콘텍스트를 저장하기 위해 스택을 변경
    add rdi, ( 19 * 8 )                     ; RDI 레지스터에 콘텍스트 자료구조의 시작 어드레스가 들어 있으므로 이것으로 19개의 레지스터 저장을 위한 어드레스를 계산
    mov rsp, rdi
    sub rdi, ( 19 * 8 )                     ; RDI 레지스터 원상 복구
    ...
    ;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;;
.LoadContext:
    mov rsp, rsi                            ; RSI 레지스터에는 콘텍스트를 복원하는데 사용할 어드레스가 들어 있으므로, 이를 스택의 Top으로 설정

    ; Context 자료구조에서 레지스터를 복원
    KLOADCONTEXT
    iretq                                   ; 스택에서 SS, RSP, RFLAGS, CS, RIP 레지스터를 꺼내 복원
