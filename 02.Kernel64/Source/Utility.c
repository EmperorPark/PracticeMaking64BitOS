...
// 64MB 이상의 위치 부터 램 크기를 체크
//      최초 부팅 과정에서 한 번만 호출 해야 함
void kCheckTotalRAMSize( void )
{
    ...
    while ( 1 )
    {
        ...
        // 0x12345678을 써서 읽었을 때 문제가 없는 곳 까지를 유효한 메모리 영역으로 인정
...
int kVSPrintf( char* pcBuffer, const char* pcFormatString, va_list ap )
{
    ...
    // 포멧 문자열의 길이를 읽어서 문자열의 길이만큼 데이터를 출력 버퍼에 출력
    ...
    for( i = 0 ; i < iFormatLength ; i++ ) 
    {
        ...
        if( pcFormatString[ i ] == '%' )
        {
            ...
            case 's':
                ...
                // 문자열의 길이만큼을 출력 버퍼로 복사하고 출력한 길이만큼 버퍼 인덱스를 이동
                ...
                // 문자 출력
            case 'c':
                // 가변 인자에 들어있는 파라미터를 문자 타입으로 변환하여 출력 버퍼에 복사하고 버퍼의 인덱스를 1 만큼 이동
            ...
            case 'd':
            case 'i':
                // 가변 인자에 들어있는 파라미터를 정수 타입으로 변환하여 출력 버퍼에 복사하고 출력한 길이만큼 버퍼의 인덱스를 이동
            ...
            case 'x':
            case 'X':
                // 가변 인자에 들어있는 파라미터를 DWORD 타입으로 변환하여 출력 버퍼에 복사하고 출력한 길이만큼 버퍼의 인덱스를 이동
                dwValue = ( DWORD ) ( va_arg( ap, DWORD ) ) & 0xFFFFFFFF;
            ...
            case 'Q':
            case 'p':
                // 가변 인자에 들어 있는 파라미터를 QWORD  타입으로 변환하여 출력버퍼에 복사하고 출력한 길이만큼 버퍼의 인덱스를 이동
