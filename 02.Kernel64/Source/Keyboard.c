...
BOOL kIsOutputBufferFull( void ) {
    // 상태 레지스터(포트 0x64)에서 읽은 값에 출력 버퍼 상태(비트 0)가
    // 1로 설정되어 있으면 출력 버퍼에 키보드가 전송한 데이터가 존재함
    if( kInPortByte( 0x64 /*상태 레지스터의 포트번호*/ ) & 0x01 ) {
        return TRUE;
    }
    return FALSE;
}
...
BOOL kIsInputBufferFull( void ) {
    ...
    if( kInPortByte( 0x64 /*상태 레지스터의 포트번호*/ ) & 0x02 ) {
        return TRUE;
    }
    return FALSE;
}
...
BOOL kActivateKeyboard( void ) {
    ...
    kOutPortByte( 0x64/*컨트롤 레지스터 포트 번호*/, 0xAE/*키보드 디바이스 활성화 커맨드*/ ); //  키보드 컨트롤러에 키보드 활성화 커맨드 전송
    ...
    kOutPortByte( 0x60/*입력 버퍼의 포트 번호*/, 0xF4/*키보드 활성화 커맨드*/ );// 키보드로 직접 활성화 커맨드 전송

    // ACK가 올 때까지 대기함
    // ACK가 오기 전에 키보드 출력 버퍼(포트 0x64)에 키 데이터가 저장 될 수 있으므로
    // 키보드에 전달된 데이터를 최대 100개까지 수신하여 ACK를 확인
    for( j = 0; j < 100; j++ ) {
        ...
        if( kInPortByte( 0x60/*출력 버퍼의 포트 번호*/ ) == 0xFA/*정상적으로 처리되었다는 응답*/ ) {
            return TRUE;
        }
    }
    return FALSE;
...
BYTE kGetKeyboardScanCode( void ) {
    ...
    while ( kIsOutputBufferFull() == FALSE ) { // 출력 버퍼(포트 0x60)의 상태를 확인하여 데이터가 차 있을 경우 TRUE, 비어 있을 경우 FALSE를 반환하는 함수
        ;
    }
...
BOOL kChangeKeyboardLED( BOOL bCapsLockOn, BOOL bNumLockOn, BOOL bScrollLockOn ) {
    ...
    // 키보드에 LED 변경 커맨드 전송하고 커맨드가 처리 될 때까지 대기
    for( i = 0; i < 0xFFFF; i++ ) {
        // 입력 버퍼(포트 0x60)가 비었으면 커맨드 전송 가능
        if( kIsInputBufferFull() == FALSE ) { // 입력 버퍼(0x60)의 상태를 확인하여 데이터가 차 있으면 TRUE, 비어 있으면 FALSE를 반환하는 함수
            break;
        }
    }
    ...
    for( j = 0; j < 100; j++ ) {
        ...
        //입력 버퍼(포트 0x60)에서 읽은 데이터가 ACK(0xFA)이면 성공
        if( kInPortByte(0x60) == 0xFA ) {
            break;
        }
    }
...
void kEnableA20Gate( void ) {
    ...
    // 컨트롤러 레지스터(포트 0x64)에 키보드 컨트롤러의 출력 포트 값을 읽는 커맨드(0xD0) 전송
    ...
    // 출력 포트(0x60)에 수신된 키보드 컨트롤러의 출력 포트 값을 읽음
...
void kReboot( void ) {
    ...
    // 입력 버퍼(포트 0x60)에 데이터가 비어 있으면 출력 포트에 값을 쓰는 커맨드와 출력 포트 데이터 전송
    for( i = 0; i < 0xFFFF; i++ ) {
        // 입력버퍼(포트 0x60)가 비어 있으면 커맨드 전송가능
        if(kIsInputBufferFull() == FALSE) {
            break;
        }
    }

    // 커맨드 레지스터에(0x64)에 0을 전달하여 프로세서를 리셋
...
BOOL kIsUseCombinedCode( BYTE bScanCode ) {
    ...
    if( kIsAlphabetScanCode( bDownScanCode ) == TRUE ) {
        // 만약 Shift 키와 Caps Lock 키 중에 하나만 눌려져 있으면 조합된 키를 되돌려 줌
    ...
    // 확장 키 코드가 수신되지 않았을 때만 처리 조합된 코드 사용
        ...
        // Num Lock 키가 눌려져 있으면, 조합된 키를 되돌려 줌
...
// 조합 키의 상태를 갱신하고 LED상태도 동기화 함
void UpdateCombinationKeyStatusAndLED( BYTE bScanCode ) {
    BOOL bDown;
    BYTE bDownScanCode;
    BOOL bLEDStatusChanged = FALSE;

    // 눌림 또는 떨어짐 상태처리, 최상위 비트(비트 7)가 1이면 키가 떨어졌음을 의미하고 0이면 눌림을 의미함
    if( bScanCode & 0x80 ) {
        bDown = FALSE;
        bDownScanCode = bScanCode & 0x7F;
    } else {
    ...
    // Caps Lock키의 스캔 코드(58)이면 Caps Lock의 상태 갱신하고 LED 상태 변경
    else if( (bDownScanCode == 58) && (bDown == TRUE) ) {
        gs_stKeyboardManager.bCapsLockOn ^= TRUE;
        bLEDStatusChanged = TRUE;
    }
    // Num Lock키의 스캔 코드(69)이면 Num Lock의 상태를 갱신하고 LED 상태 변경
    else if( ( bDownScanCode == 69 ) && ( bDown == TRUE ) ) {
        gs_stKeyboardManager.bNumLockOn ^= TRUE;
        bLEDStatusChanged = TRUE;
    }
    // Scroll Lock키의 스캔 코드(70)이면 Scroll Lock의 상태를 갱신하고 LED 상태 변경
...
BOOL kConvertScanCodeToASCIICode( BYTE bScanCode, BYTE* pbASCIICode, BOOL* pbFlags ) {
    ...
    // 확장 키 코드가 들어 왔을 때, 실제 키 값은 다음에 들어오므로 플래그 설정만 하고 종료
    ...
    // 조합키 눌림 또는 떨어짐 상태를 갱신
    UpdateCombinationKeyStatusAndLED( bScanCode ); // 현재 입력된 스캔 코드를 이용하여 조합 키 상태와 LED 상태를 갱신하는 함수
    return TRUE;
}