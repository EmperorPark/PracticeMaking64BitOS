...
{
    WORD wLimit;
    QWORD qwBaseAddress; // 16바이트 어드레스 정렬을 위해 추가
    WORD wPading;
    DWORD dwPading;
} GDTR, IDTR;
...
typedef struct kGDTEntry16Struct // 16바이트 크기의 TSS 세그먼트 디스크립터를 위한 자료구조
{
    WORD wLowerLimit;
    WORD wLowerBaseAddress;
...
{
    WORD wLowerBaseAddress;
    WORD wSegmentSelector; // 3비트 IST, 5비트 0
    BYTE bIST; // 4비트 Type, 1비트 0, 2비트 DPL, 1비트 P
    BYTE bTypeAndFlags;
