...
%macro KSAVECONTEXT 0       ; 파라미터를 전달받지 않는 KSAVECONTEXT 매크로 정의
    ...
    push rbp
    mov rbp, rsp    ; 에러 코드를 포함하는 예외가 발생했을 때, 현재 스택의 위치를 RBP 레지스터에 저장하여 스택에 삽입된 에러코드에 RSP + 8로 접근할 수 있도록 함
    push rax
    push rbx
    push rcx
    push rdx
...
kISRDivideError:
    ...
    mov rdi, 0 ; 핸들러에 백터 번호를 넘겨주려고 첫 번째 파라미터의 역할을 하는 RDI 레지스터에 삽입
...
kISRKeyboard:
    ...
    ; 핸들러에 인터럽트 번호를 삽입하고 핸들러 호출
    mov rdi, 33
    call kKeyboardHandler   ; 키보드 처리를 위해 작성된 전용 핸들러 함수를 호출
