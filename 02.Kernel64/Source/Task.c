...
TCB* kAllocateTCB( void )
{
    ...
    // 상위 32비트를 0이 아닌 다른 값으로 설정해서 할당된 TCB로 설정
    // 중복을 피하기 위해 iAllocateCount를 TCB ID에 OR 수행 
...
TCB* kCreateTask( QWORD qwFlags, QWORD qwEntryPointAddress )
{
    ...
    // TCB를 설정한 후 준비 리스트에 삽입하여 스케줄링이 될 수 있도록 함
...
void kSetUpTask( TCB* pstTCB, QWORD qwFlags, QWORD qwEntryPointAddress, void* pvStackAddress, QWORD qwStackSize )
{
    ...
    pstTCB->stContext.vqRegister[ TASK_RSPOFFSET ] = ( QWORD ) pvStackAddress + qwStackSize; // 태스크가 생성 되었을 때 스택은 비어있으므로, 스택의 Top을 스택 영역의 끝으로 옮김
    pstTCB->stContext.vqRegister[ TASK_RBPOFFSET ] = ( QWORD ) pvStackAddress + qwStackSize;

    // 세그먼트 셀렉터 설정
    pstTCB->stContext.vqRegister[ TASK_CSOFFSET ] = GDT_KERNELCODESEGMENT; // 태스크가 커널모드에서 실행되므로 커널 코드 세그먼터 디스크립터를 설정
    ...
    pstTCB->stContext.vqRegister[ TASK_RFLAGSOFFSET ] |= 0x0200; // 인터럽트를 활성화 하기 위해 RFLAGS 레지스터의 IF 비트(비트 9)를 1로 설정
...
void kSchedule( void )
{
    ...
    // 전환할 태스크가 있어야 함
    if( kGetListCount( &( gs_stScheduler.stReadyList ) ) == 0 ) // 준비 리스트에 태스크가 없으면 인터럽트를 비활성화 할 필요가 없으므로 미리 확인. 불필요한 인터럽트 불가 설정을 막기 위함
    {
        return ;
    }

    // 전환하는 도중 인터럽트가 발생하여 태스크 전환이 또 일어나면 곤란하므로 전환하는 동안 인터럽트가 발생하지 못하도록 설정
    bPreviousFlag = kSetInterruptFlag( FALSE ); // 이전 인터럽트 플래그를 저장하고, 인터럽트를 비활성화
    // 실행할 다음 태스크를 얻음
    ...
    kSwitchContext( &( pstRunningTask->stContext ), &( pstNextTask->stContext ) ); // 현재 태스크의 콘텍스트와 다음 태스크의 콘텍스트를 전달하여 콘텍스트 전환 수행
...
BOOL kScheduleInInterrupt( void )
{
    ...
    //======================================================================
    pcContextAddress = ( char* ) IST_STARTADDRESS + IST_SIZE - sizeof( CONTEXT ); // IST도 스택이므로 콘텍스트가 IST 영역의 끝부분부터 저장됨. IST 영역의 끝에서 콘텍스트의 크기를 빼서 콘텍스트의 시작 어드레스를 계산
