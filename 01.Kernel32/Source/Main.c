...
// IA-32e 모드용 커널 영역(1Mbytes ~ 6Mbytes)을 0으로 초기화
BOOL kInitializeKernel64Area(void) {
    ...
    // 마지막 어드레스인 0x600000(6MB)까지 루프를 돌면서 4Bytes씩 0으로 채움
    while ((DWORD) pdwCurrentAddress < 0x600000)
    {
        *pdwCurrentAddress = 0x00;

        // 0으로 지정한 후 다시 읽었을 때 0이 나오지 않으면 해당 어드레스를 사용하는데 문제가 생긴것으로 더이상 진행하지 않고 정료
...
BOOL kIsMemoryEnough(void) {
    ...
    while ((DWORD) pdwCurrentAddress < 0x4000000 )
    {
        *pdwCurrentAddress = 0x12345678;
        // 0x12345678로 저장 한 후 다시 읽었을 때 0x12345678이 나오지 않으면 해당 어드레스를 사용하는데 문제가 생긴 것이므로 더이상 진행하지 않고 종료
