...
TOTALSECORCOUNT     equ 1024    ; 가상 OS의 총 섹터 수 ; 최대 1152(0x90000bytes)까지 가능
...
START:
    ...
    %rep TOTALSECORCOUNT    ; TOTALSECOTRCOUNT에 저장된 값만큼 아래 코드를 반복
        ...
        ; 마지막 섹터면 더 수행할 섹터가 없으므로 무한 루프 수행, 그렇지 않으면 다음 섹터로 이동해서 코드 수행
        %if i == TOTALSECORCOUNT                ; i가 TOTALSECORCOUNT와 같다면, 즉 마지막 섹터이면
            jmp $                               ; 현재 위치에서 무한 루프 수행
        %else                                   ; 마지막 섹터가 아니면
            jmp (0x1000 + i * 0x20): 0x0000     ; 다음섹터 오프셋으로 이동
        %endif                                  ; if문의 끝

        times (512 - ( $ - $$ ) % 512)  db 0x00     ; $ 현재 라인의 Address
